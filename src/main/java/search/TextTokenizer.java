...
/**
 * Iterator class which returns token strings from the input string. It must be
 * initialized with a regular expression representing a word. Returned tokens
 * will alternate between words (i.e. that match the regular expression) and
 * intervening strings that are not words.
 * <p>
 * 
 * The method isWord(String) can be used to check whether a given token is a
 * word or an intervening token.
 * 
...
public class TextTokenizer implements Iterator<String> {
	...
	/**
	 * Returns true if more tokens are available, i.e. if end-of-file has not been
	 * reached.
	 */
	...
	/**
	 * Returns the next token if any more tokens are available. Otherwise returns
	 * null.
	 */
	public String next() {
		if (nextPunctuation == null && nextWord == null) {
